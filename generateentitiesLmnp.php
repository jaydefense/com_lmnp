...
while($row = mysql_fetch_array($temp)) {
  ...
  while($row2 = mysql_fetch_array($temp2)) {
        ...
        $variables .= sprintf('%spublic $%s = null;%s',$doublespace,strtolower($row2['Field']),$linebreak);
        ...
        if($row2['Extra'] != 'auto_increment') {
          ...
          $required = 1;
          $setters .= sprintf('%sfunction set%s($newvalue) {%s',$doublespace,ucfirst(strtolower($row2['Field'])),$linebreak);
          ...
          if($row2['Null'] == 'YES') {
            $setters .= sprintf('%sif(is_null($newvalue)) {%s',$quadspace,$linebreak); 
                $setters .= sprintf('%s$this->%s = $newvalue;%s',$septspace,strtolower($row2['Field']),$linebreak);
        ...
        $setters .= sprintf('%sif(is_null($newvalue)) {%s',$quadspace,$linebreak); 
            $setters .= sprintf('%sthrow new Exception("Value cannot be null - %s");%s}%s',$septspace,ucfirst(strtolower($row2['Field'])),"\r\n".$quadspace,$linebreak);
          }
          
          // if text
          if($row2['Type'] == 'text') {
            $getters .= sprintf('%sfunction get%s() {%s',$doublespace,ucfirst(strtolower($row2['Field'])),$linebreak);
            $getters .= sprintf('%sreturn $this->%s;%s}%s',$quadspace,strtolower($row2['Field']),"\r\n".$doublespace,$linebreak);
            ...
            $setters .= sprintf('%s$this->%s = $newvalue;%s',$quadspace,strtolower($row2['Field']),$linebreak);
          ...
          if(strpos($row2['Type'],'int') === 0) {
            $getters .= sprintf('%sfunction get%s() {%s',$doublespace,ucfirst(strtolower($row2['Field'])),$linebreak);
            $getters .= sprintf('%sreturn (int)$this->%s;%s}%s',$quadspace,strtolower($row2['Field']),"\r\n".$doublespace,$linebreak);
            ...
            $setters .= sprintf('%sif(gettype($newvalue) != "integer") {%s',$quadspace,$linebreak); 
                ...
                $setters .= sprintf('%s$this->%s = (int)$newvalue;%s',$quadspace,strtolower($row2['Field']),$linebreak);
                $setters .= sprintf('%s}%s',$doublespace,$linebreak);
                $values[] = '?';
                $inserttype .= 'i';
                $type = 'integer';
        ...
        $getters .= sprintf('%sfunction get%s() {%s',$doublespace,ucfirst(strtolower($row2['Field'])),$linebreak);
          ...
          if(strpos($row2['Type'],'datetime') === 0) {
            $getters .= sprintf('%sfunction get%s() {%s',$doublespace,ucfirst(strtolower($row2['Field'])),$linebreak);
        $getters .= sprintf('%sreturn strtotime($this->%s);%s}%s',$quadspace,strtolower($row2['Field']),"\r\n".$doublespace,$linebreak);
          
            $setters .= sprintf('%sif(gettype($newvalue) != "integer") {%s',$quadspace,$linebreak); 
                $setters .= sprintf('%sthrow new Exception("Value not integer - ".$newvalue);%s}%s',$septspace,"\r\n".$quadspace,$linebreak);           
                $setters .= sprintf('%s$this->%s = date("Y-m-d H:i:s",(int)$newvalue);%s',$quadspace,strtolower($row2['Field']),$linebreak);
            $setters .= sprintf('%s}%s',$doublespace,$linebreak);
                $values[] = '?';
                $inserttype .= 's';
                $type = 'integer';
        ...
        $setters .= sprintf('%s$this->%s = $newvalue;%s',$quadspace,strtolower($row2['Field']),$linebreak);
            ...
            $warnings[] = sprintf('Unknown type "%s" may not save correctly. You can ignore this if you know how to handle the database type yourself. CLASSNAME = %s',strtolower($row2['Field']),$row[0]);
            $setters .= sprintf('%s// !!!WARNING!!! Unknown type "%s" may not save correctly. You can ignore this if you know how to handle the database type yourself.%s',$quadspace,strtolower($row2['Field']),$linebreak);
                $setters .= sprintf('%s$this->%s = $newvalue;%s',$quadspace,strtolower($row2['Field']),$linebreak);
            $setters .= sprintf('%s}%s',$doublespace,$linebreak);           
                $values[] = '?';
                $inserttype .= 's';
  ...
  foreach ($valuesname as $i) {
    $partialvalues .= '$this->'.strtolower($i).',';
  }
  ...
  $updatequery2 = sprintf('$query->execute(array(%s,$this->%s));',$partialvalues,strtolower($primarykey));
  $deletequery = sprintf('$query = $db->prepare("DELETE FROM `%s` WHERE `%s`=? LIMIT 1;");',$row[0],$primarykey);
  $deletequery2 = sprintf('$query->execute(array($this->%s));',strtolower($primarykey));
  ...
  $filecontents .= sprintf('%sif(is_null($this->%s)) {%s',$quadspace,strtolower($primarykey),$linebreak);
  $filecontents .= sprintf('%s%s%s',$septspace,$insertquery,$linebreak);
  $filecontents .= sprintf('%s%s%s',$septspace,$insertquery2,$linebreak);
  $filecontents .= sprintf('%s$this->%s = $db->lastInsertId();%s',$septspace,strtolower($primarykey),$linebreak);
  ...
  $selectdo = '';
  foreach ($valuesname as $i) {
    $selectdo .= sprintf('%s$this->%s = $row_data["%s"];%s',$octspace,strtolower($i),$i,$linebreak);
  }
  ...
  $filecontents .= sprintf('%s$this->%s = $row_data["%s"];%s',$octspace,strtolower($primarykey),$primarykey,$linebreak);
